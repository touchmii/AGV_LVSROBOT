...
#define TB_WHITE VT_F_WHITE
#define TB_YELLOW VT_F_YELLOW
#define TB_BLUE VT_B_BLUE
#define TB_GREEN VT_F_GREEN
#define TB_CYAN VT_F_CYAN
#define TB_RED VT_B_RED
#define TB_MAGENTA VT_F_WHITE
#define TB_BOLD 1
#define TB_DEFAULT VT_B_BLACK
...
struct key K_BACKSPACE[] = {{44,4,' '},{45,4,"🔙"},{46,4,' '},STOP};
...
struct key K_ARROW_UP[] = {{54,10,'('},{55,10,"↑"},{56,10,')'},STOP};
struct key K_K_1[] = {{65,10,'1'},STOP};
struct key K_K_2[] = {{68,10,'2'},STOP};
struct key K_K_3[] = {{71,10,'3'},STOP};
struct key K_K_ENTER[] = {{74,10,' '},{74,11,"↩️"},{74,12,' '},STOP};
...
struct key K_ARROW_LEFT[] = {{50,12,'('},{51,12,"←"},{52,12,')'},STOP};
struct key K_ARROW_DOWN[] = {{54,12,'('},{55,12,"↓"},{56,12,')'},STOP};
struct key K_ARROW_RIGHT[] = {{58,12,'('},{59,12,"→"},{60,12,')'},STOP};
...
void print_tb(const char *str, int x, int y, uint16_t fg, uint16_t bg)
{
	while (*str) {
		...
		// tb_chgange_cell(x, y, uni, fg, bg);
		vt_set_bg_color(bg);
		vt_set_font_color(fg);
		vt_draw_char_at(x, y, uni);
		x++;
	}
}
...
void draw_key(struct key *k, uint16_t fg, uint16_t bg)
{
	while (k->x) {
		...
		vt_set_font_color(fg);
		if (k->ch < 128)  {
			vt_draw_char_at(k->x+2, k->y+4, k->ch);
		} else {

			vt_draw_utf8_at(k->x+2, k->y+4, k->ch);
		}
		k++;
	}
}
...
{
	// int i;
	// vt_draw_char_at(0, 0, 0x250C);
	// vt_draw_char_at(79, 0, 0x2510);
	// vt_draw_char_at(0, 23, 0x2514);
	// vt_draw_char_at(79, 23, 0x2518);

	// for (i = 1; i < 79; ++i) {
	// 	vt_draw_char_at(i, 0, 0x2500);
	// 	vt_draw_char_at(i, 23, 0x2500);
	// 	vt_draw_char_at(i, 17, 0x2500);
	// 	vt_draw_char_at(i, 4, 0x2500);
	// }
	// for (i = 1; i < 23; ++i) {
	// 	vt_draw_char_at(0, i, 0x2502);
	// 	vt_draw_char_at(79, i, 0x2502);
	// }
	// vt_draw_char_at(0, 17, 0x251C);
	// vt_draw_char_at(79, 17, 0x2524);
	// vt_draw_char_at(0, 4, 0x251C);
	// vt_draw_char_at(79, 4, 0x2524);
	// for (i = 5; i < 17; ++i) {
	// 	//yellow
	// 	vt_draw_char_at(1, i, 0x2588);
	// 	vt_draw_char_at(78, i, 0x2588);
	// }
	...
	draw_key(K_SPACE, TB_WHITE, TB_RED);
	...
	vt_set_font_color(VT_F_PURPLE);
	vt_draw_str_at(33, 1, "Keyboard demo!");
	vt_draw_str_at(21, 2, "(press CTRL+X and then CTRL+Q to exit)");
	vt_draw_str_at(15, 3, "(press CTRL+X and then CTRL+C to change input mode)");
...
// #include <finsh.h>
// FINSH_FUNCTION_EXPORT (tb_keyboard, "termbox keyboard demo");
